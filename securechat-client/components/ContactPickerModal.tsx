...
export default function ContactPickerModal({ isOpen, onClose, onSelectContact }: ContactPickerModalProps) {
  const { user } = useAuth();
  ...
  const hasExistingConversation = (contactUserId: string): Conversation | undefined => {
    return conversations.find(
      (conv) =>
        conv.participantUserIds.length === 2 &&
        conv.participantUserIds.includes(contactUserId) &&
          ...
          {isLoading ? (
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredContacts.map((contact) => {
                const existingConv = hasExistingConversation(contact.userId);
                const displayName = contact.nickname || contact.displayName;
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-3 p-4 text-left hover:bg-gray-50 dark:hover:bg-gray-700"
                  >
                    {/* Avatar */}
